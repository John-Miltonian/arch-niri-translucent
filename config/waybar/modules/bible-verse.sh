#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════════
#   WAYBAR BIBLE VERSE MODULE
#   Returns a daily scripture for display in Waybar
# ═══════════════════════════════════════════════════════════════════════════════
...
# Use day of year for daily verse, with hour for variety
if command -v date &> /dev/null; then
    day_of_year=$(date +%j)
    hour=$(date +%H)
    index=$(((day_of_year * 24 + hour) % ${#verses[@]}))
else
    index=$((RANDOM % ${#verses[@]}))
fi

# Output as JSON for Waybar
echo "{\"text\":\"${verses[$index]}\",\"tooltip\":\"📖 Daily Scripture - Click to refresh\",\"class\":\"bible-verse\"}"
